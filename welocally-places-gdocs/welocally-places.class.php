...
if ( !class_exists( 'WelocallyPlaces' ) ) {
	...
	class WelocallyPlaces {
		const VERSION = '1.2.19.DEV';
		const OPTIONNAME = 'wl_places_options';
		

		function __construct() {
			add_shortcode('welocally', array ($this,'handleWlPlacesShortcode'));		
		...
		public function loadDomainStylesScripts() {
			...
			if(!empty($options['gmaps_key'])){
				wp_enqueue_script('google-maps' , 'https://maps.google.com/maps/api/js?key='.$options['gmaps_key'].'&sensor=false' , false , '3');							
			} else {
			...
			//welocally
			wp_enqueue_script('wl_places', WP_PLUGIN_URL.'/welocally-places/resources/javascripts/wl_places_dev_all.js', array('jquery'), WelocallyPlaces::VERSION);
			
			global $wp_styles;
			
			if(empty($options['theme']))
				$options['theme'] = 'welocally';
					
			wp_enqueue_style( 'wl_places',WP_PLUGIN_URL.'/welocally-places/themes/'.$options['theme'].'/css/wl_places.css', array(), WelocallyPlaces::VERSION, 'screen' );
			wp_register_style('wl_places-ie-only', WP_PLUGIN_URL.'/welocally-places/themes/'.$options['theme'].'/css/ie/wl_places.css');
			$wp_styles->add_data('wl_places-ie-only', 'conditional', 'IE');
			wp_enqueue_style('wl_places-ie-only');
						
			wp_enqueue_style( 'wl_places_place',WP_PLUGIN_URL.'/welocally-places/themes/'.$options['theme'].'/css/wl_places_place.css', array(), WelocallyPlaces::VERSION, 'screen' );
			wp_register_style('wl_places_place-ie-only', WP_PLUGIN_URL.'/welocally-places/themes/'.$options['theme'].'/css/ie/wl_places_place.css');
			$wp_styles->add_data('wl_places_place-ie-only', 'conditional', 'IE');
			wp_enqueue_style('wl_places_place-ie-only');	
			
			wp_enqueue_style( 'wl_places_multi',WP_PLUGIN_URL.'/welocally-places/themes/'.$options['theme'].'/css/wl_places_multi.css', array(), WelocallyPlaces::VERSION, 'screen' );
			wp_register_style('wl_places_multi-ie-only', WP_PLUGIN_URL.'/welocally-places/themes/'.$options['theme'].'/css/ie/wl_places_multi.css');
		...
		function handleWlPlacesShortcode($attrs, $content = null) {
			...
			$t = new StdClass();
			$t->uid = uniqid();
			$t->collectionKey = $collection_key;
			$t->placeId = $place_id;
			$t->provider = $provider;
			$t->type = $type;
			$t->showLetters = $show_letters;
			$t->showSelection = $show_selection;
			
			$template='';
			if($t->type == 'place' && $t->provider == 'google')
				$template = '/views/google-place-template.php';	
			else if($t->type == 'places' && $t->provider == 'google')
				$template = '/views/google-places-template.php';	
			else
				$template = '/views/template-empty.php';
        ...
        public function getOptions() {
            $options = get_option(WelocallyPlaces::OPTIONNAME, array());
            
            $default_theme = 'welocally'; 
                      
            //theme
	    	if ( !array_key_exists( 'theme', $options ) ) { $options[ 'theme' ] = $default_theme; $changed = true; }
	    	if ( !array_key_exists( 'gmaps_key', $options ) ) { $options[ 'gmaps_key' ] = null; $changed = true; }
