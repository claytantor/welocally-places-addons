...
function WELOCALLY_PlaceMobile (cfg) {	
...
WELOCALLY_PlaceMobile.prototype.initCfg = function(cfg) {
	var errors = [];
	if (!cfg) {
...
WELOCALLY_PlaceMobile.prototype.makeWrapper = function() { 
	// Build Widget
	var _instance = this;
	...
	jQuery(wrapper).attr('class','welocally_place_mobile');
	jQuery(wrapper).attr('id','welocally_place_mobile_'+_instance.cfg.id);
...
WELOCALLY_PlaceMobile.prototype.load = function(placeJson) {
	var _instance = this;
...
WELOCALLY_PlaceMobile.prototype.initMapForPlace = function(place, map_canvas) {
...
WELOCALLY_PlaceMobile.prototype.setMapEvents = function(map){
...
WELOCALLY_PlaceMobile.prototype.makePlaceContent = function(selectedPlace, cfg) {
	
	var _instance= this;
	
	var placeWrapper = jQuery('<div class="wl_places_place_wrapper"></div>');
	
	jQuery('#wl_placepost_map').find('.wl_map_actions').hide();
	...
	if(selectedPlace.properties.website != null && 
		...
		//neither http or https
		if(selectedPlace.properties.website.indexOf('http://') == -1 && 
				selectedPlace.properties.website.indexOf('https://') == -1 ) {
			website = 'http://'+selectedPlace.properties.website;				
		}
		
		jQuery('#wl_place_post_btn_web').attr('href',website);
		jQuery('#wl_place_post_btn_web').show();
	...
	if(selectedPlace.properties.city != null && 
		selectedPlace.properties.province != null){
			...
			var directions = 'http://maps.google.com/maps?f=d&source=s_q&hl=en&geocode=&q='+qVal;
			
			jQuery('#wl_place_post_btn_directions').attr('href',directions);
			jQuery('#wl_place_post_btn_directions').show();
...
//for place selectors
WELOCALLY_PlaceMobile.prototype.show = function(selectedPlace) {	
	this.wrapper.html(this.makePlaceContent(selectedPlace, this.cfg));	  
	jQuery(this.wrapper).show();
};


WELOCALLY_PlaceMobile.prototype.refreshMap = function(searchLocation) {
	var _instance = this;
...
//use map status area
WELOCALLY_PlaceMobile.prototype.getStatusArea = function (){
	var _instance = this;
