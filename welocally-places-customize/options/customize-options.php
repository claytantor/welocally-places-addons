		...
		var submitApplication = function(data) {
			if(data.success == 'true'){
				...
				if(data.customize == 'off') {
					jQuery('#theme_options').show();
				}
				if (data.customize == 'on') {
					jQuery('#theme_options').hide();
				}
...
<h2>Welocally Places Customization Options</h2>
	<form method="post" action="<?php echo get_bloginfo( 'wpurl' ).'/wp-admin/admin.php?page=welocally-places-customize-options' ?>">
		...
		<ul id="theme_options" <?php if($options['theme_customize'] == 'off'):?> style="display:none" <?php else:?> style="display: block;" <?php endif;?>>
